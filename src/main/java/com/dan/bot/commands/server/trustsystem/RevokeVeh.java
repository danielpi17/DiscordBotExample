...
public class RevokeVeh extends DiscordSlashCmd {
    public RevokeVeh() {
        super("revokeveh", "Revoke Vehicle", SlashUtil.newArg("vehicle", "Vehicle spawncode", true), SlashUtil.newArg(SlashCommandOptionType.USER, "user", "user", true));
    }
    @Override
    public void execute(SlashCommandCreateEvent e, SlashCommandInteraction i) {
        try {
            ...
            if(!alrhas) {
                embed.setDescription("User is not trusted!");
            } else {
                NetworkHandler.trustSystem.sendMessage(EmbedTools.info("NETWORK")
                        .setDescription(i.getUser().getMentionTag() + " revoked access to " + user.getMentionTag() + " from " + vehicle)
                );
                DB.execute("DELETE FROM vehtrustuser WHERE spawncode = ? AND discordid = ?", vehicle, "discord:" + user.getIdAsString());
                embed.setDescription("Revoked trust from user " + user.getMentionTag() + " on vehicle " + vehicle);
            }

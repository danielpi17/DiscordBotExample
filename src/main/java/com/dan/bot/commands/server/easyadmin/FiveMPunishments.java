...
public class FiveMPunishments extends DiscordSlashCmd {
    public FiveMPunishments() {
        super("fivem-punishments", "FiveM Punishments", ServerPermissions.FIVEM_PUNISHMENTS,
    ...
    public void execute(SlashCommandCreateEvent e, SlashCommandInteraction i) {
        ...
        String timezone = "UTC";
        ResultSet rs = DB.query("SELECT * FROM punishments WHERE server = 'FIVEM' AND user = ? ORDER BY timestamp ASC", user.getIdAsString());
        ...
        boolean rp = false;
            ...
            try {
                duration = Duration.toMaxTime((Long.parseLong(duration)-Long.parseLong(timestamp))*1000) + " <t:" + Long.parseLong(duration) + ":F> (<t:" + Long.parseLong(duration) + ":R>)";
            } catch (Exception ignored) {}

            if(timestamp.equals("0")) {
                timestampPrint = "";
            } else {
                timestampPrint = "<t:" + timestamp + ":F> (<t:" + timestamp + ":R>)";
            }

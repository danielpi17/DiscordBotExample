...
public class DiscordPunishments extends DiscordSlashCmd {
    public DiscordPunishments() {
        super("discord-punishments", "Discord Punishments", ServerPermissions.DISCORD_PUNISHMENTS,
    ...
    public void execute(SlashCommandCreateEvent e, SlashCommandInteraction i) {
        ...
        String timezone = "UTC";
        ResultSet rs = DB.query("SELECT * FROM punishments WHERE server = ? AND user = ? ORDER BY timestamp ASC", i.getServer().get().getIdAsString(), user.getIdAsString());
